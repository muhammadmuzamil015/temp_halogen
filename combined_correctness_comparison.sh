...
QEMU_AARCH64_RESULTS_CSV="correctness_results_qemu-aarch64.csv"
QEMU_RISCV64_RESULTS_CSV="correctness_results_qemu-riscv64.csv"
BPIF3_RESULTS_CSV="correctness_results_bpif3.csv"
RASPI4_RESULTS_CSV="correctness_results_raspi4.csv"
...
if [[ ! -s "$TEMP_AARCH64_RISCV64" || ! -s "$TEMP_AARCH64_BPIF3" || ! -s "$TEMP_AARCH64_RASPI4" ]]; then
...
echo "testname,qemu_riscv64_status,bpif3_status,raspi4_status" > $OUTPUT_CSV

# Initialize counters for passed and failed tests
declare -A pass_count=([qemu_riscv64_status]=0 [bpif3_status]=0 [raspi4_status]=0)
declare -A fail_count=([qemu_riscv64_status]=0 [bpif3_status]=0 [raspi4_status]=0)
...
while IFS=, read -r testname qemu_riscv64_status; do
    ...
    for status in qemu_riscv64_status bpif3_status raspi4_status; do
...
echo "Passed,${pass_count[qemu_riscv64_status]},${pass_count[bpif3_status]},${pass_count[raspi4_status]}" >> $OUTPUT_CSV
echo "Failed,${fail_count[qemu_riscv64_status]},${fail_count[bpif3_status]},${fail_count[raspi4_status]}" >> $OUTPUT_CSV

# Clean up temporary files
rm -f "$TEMP_AARCH64_RISCV64" "$TEMP_AARCH64_BPIF3" "$TEMP_AARCH64_RASPI4"
