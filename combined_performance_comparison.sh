...
SPIKE_RESULTS_CSV="performance_results_spike.csv"
...
echo "testname,qemu_aarch64 (us),qemu_riscv64 (us),spike (us),bpif3 (us),raspi4 (us)" > $OUTPUT_CSV

# Read results into associative arrays for each target
declare -A qemu_riscv64_times
declare -A spike_times
...
read_csv_into_array "$SPIKE_RESULTS_CSV" spike_times
...
while IFS=, read -r testname aarch64_time; do
    ...
    spike_time=${spike_times["$testname"]:=""}  # Default to empty string if not found
    ...
    echo "$testname,$aarch64_time,$riscv64_time,$spike_time,$bpif3_time,$raspi4_time" >> $OUTPUT_CSV
